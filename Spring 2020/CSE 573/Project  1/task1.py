...
def convolve2d(img, kernel):
    ...
    img_conv = np.zeros((len(img), len(img)))
    ...
    for i, row in enumerate(img):
        for j, col in enumerate(img):
            sum_img = 0
            cropped_image = utils.crop(padded_image, i, i+3, j, j+3)
            img_mul = utils.elementwise_mul(cropped_image, kernel_flip)
            for row in img_mul:
                for col in row:
                    sum_img += col
            img_conv[i][j] = sum_img
...
def normalize(img):
    ...
    img_norm = copy.deepcopy(img)
    min_img = sys.maxsize
    max_img = -min_img - 1
    for i, row in enumerate(img_norm):
        for j, col in enumerate(row):
            max_img = max(max_img, img_norm[i][j])
            min_img = min(min_img, img_norm[i][j])
            
    for i, row in enumerate(img_norm):
        for j, col in enumerate(row):
            img_norm[i][j] = (img_norm[i][j] - min_img) / (max_img - min_img)
...
def edge_magnitude(edge_x, edge_y):
    ...
    edge_mag = np.zeros((len(edge_x), len(edge_x)))
    for i, row in enumerate(edge_x):
        for j, col in enumerate(row):
            edge_mag[i][j] = (edge_x[i][j] ** 2 + edge_y[i][j] ** 2) ** 0.5
               
    max_edge_mag = np.max(edge_mag) 
    for i, row in enumerate(edge_mag):
        for j, col in enumerate(row):
            edge_mag[i][j] /= max_edge_mag
